### This script can be used to produce Bayesian model rate of spread (ROS) predictions.
### The script require two data files provided with the supplementary meterial (user needs to define their location on their local machine)
### THe user can change the input values of windspeed

library(ggplot2)



### The output folder where table from running JAGS (provided as file)
dat.fold <- "D:\\model_data\\"

#load in the JAGS output, which contains the random sample of regression slopes (b0,b1,b2,b3) for each predictor
...
## the is required as predictor values were standardized for model fitting
model.data.summary.4<- read.csv(paste0(dat.fold,"dat.sum.4.csv"))

###define some values of the predictor values
...
#standardize (mean and sd) the values for sampling of posterior distribution
...
### define the function to predict mean ROS

## ROS MAX was defined in sigmoid equation in the analysis was 15 km/h. Do not change
ROS.MAX <- 15
fn_predict.4 <- function(x1,x2,x3, b0, b1,b2,b3,z) { 
...
dat.post$trend <- fn_predict.4(ws_stnd,rh_stnd,soilmois_stnd,dat.post$b0,dat.post$b1,dat.post$b2,dat.post$b3,dat.post$z)

#create posterior distribution using mean predictor ROS and phi parameter to sample gamma distribution
...
print(paste0(round(selection1,1)," % of the predictive distribution is > ", ROS_value, " km/h"))
...
print(paste0(round(selection2,1)," % of the predictive distribution is < ", ROS_value, " km/h"))
...
print(paste0(round(selection3,1)," % of the predictive distribution is > ", ROS_value_lower, " km/h & < ", ROS_value_upper, " km/h"))
